...
// getStateFile returns the filename to use to track cirlog state.
//
...
//
// If cirlog is asked to tail multiple files, we will only respect the
...
func getStateFile(conf Config, filename string, numFiles int) string {
	...
	if conf.Options.StateFile != "" {
		...
		if numFiles == 1 && (os.IsNotExist(err) || (err == nil && !info.IsDir())) {
			...
			logrus.Debugf("Couldn't write to --tail.statefile=%s, writing cirlog state for %s to $TMPDIR (%s) instead.",
