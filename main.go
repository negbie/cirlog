...
import (
	...
	"github.com/negbie/cirlog/httime"
...
// GlobalOptions has all the top level CLI flags that cirlog supports
type GlobalOptions struct {
	Host                string   `hidden:"true" long:"api_host" description:"Host to send to" default:"localhost:9060"`
	TailSample          bool     `hidden:"true" description:"When true, sample while tailing. When false, sample post-parser events"`
	ConfigFile          string   `short:"c" long:"config" description:"Config file for cirlog in INI format." no-ini:"true"`
	SampleRate          uint     `short:"r" long:"samplerate" description:"Only send 1 / N log lines" default:"1"`
	NumSenders          uint     `short:"P" long:"poolsize" description:"Number of concurrent Senders" default:"80"`
	...
	DebugOut            bool     `long:"debug_stdout" description:"Instead of sending events print them to STDOUT for debugging"`
	StatusInterval      uint     `long:"status_interval" description:"How frequently, in seconds, to print out summary info" default:"60"`
	Backfill            bool     `long:"backfill" description:"Configure cirlog to ingest old data in order to backfill. Sets the correct values for --backoff, --tail.read_from, and --tail.stop"`
	...
	DeterministicSample string   `long:"deterministic_sampling" description:"Specify a field to deterministically sample on, i.e., every concurrent cirlog instance will sample 1/N based on content."`
	...
	Syslog syslog.Options `group:"Syslog Parser Options" namespace:"syslog"`
}
...
func handleOtherModes(fp *flag.Parser, modes OtherModes) {
	if modes.Version {
		fmt.Println("cirlog version", version)
		os.Exit(0)
	}
	if modes.Help {
...
func usage() {
	fmt.Print(`
Usage: cirlog -p <parser> -k <writekey> -f </path/to/logfile> -d <mydata> [optional arguments]
